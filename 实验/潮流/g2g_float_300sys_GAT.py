...
if __name__ == "__main__":
    ...
    for i, layer in enumerate(model.layers):
        ...
        np.savez(path+r'/saved_models/300-system/GAT_noPI_weights/'+f'layer_{i}_weights.npz', *weights)
...
if __name__ == "__main__":
    model = create_model()
    for i, layer in enumerate(model.layers):
        weights = np.load(path+r'/saved_models/300-system/GAT_noPI_weights/'+f'layer_{i}_weights.npz')
        layer.set_weights([weights[f'arr_{j}'] for j in range(len(weights))])
    
    total_sample_num = 500  
    sample_for_each_iter = 100
    i = 0
    dataset = "testSet"
