...
def create_model():
    ...
    h_hidden_reshaped = tf.reshape(h_hidden, [-1, 1581*6])
    #h_hidden = GAT(units=6, num_heads=8)([h_hidden,A])
    h_mlp_in = tf.keras.layers.Dense(2048, activation='relu',name='dense1',kernel_regularizer=l2(0.001))(h_hidden_reshaped)
    h_mlp_hidden = tf.keras.layers.Dense(2048, activation='relu',name='dense2',kernel_regularizer=l2(0.001))(h_mlp_in)
    h_mlp_out = tf.keras.layers.Dense(1581*6, activation='relu',name='dense3',kernel_regularizer=l2(0.001))(h_mlp_hidden)
    h_mlp_out = tf.reshape(h_mlp_out, [-1,1581, 6])
    h_mlp_out = tf.keras.layers.LayerNormalization(epsilon=1e-6,name='norm2')(h_mlp_out + h_hidden)

    h_hidden = GAT(units=6, num_heads=8,name='GAT3')([hin,A])   
    ...
    h_hidden_reshaped = tf.reshape(h_hidden, [-1, 1581*6])
    #h_hidden = GAT(units=6, num_heads=8)([h_hidden,A])
    h_mlp_in = tf.keras.layers.Dense(2048, activation='relu',name='dense4',kernel_regularizer=l2(0.001))(h_hidden_reshaped)
    h_mlp_hidden = tf.keras.layers.Dense(2048, activation='relu',name='dense5',kernel_regularizer=l2(0.001))(h_mlp_in)
    h_mlp_out = tf.keras.layers.Dense(1581*6, activation='relu',name='dense6',kernel_regularizer=l2(0.001))(h_mlp_hidden)
    h_mlp_out = tf.reshape(h_mlp_out, [-1,1581, 6])
    h_mlp_out = tf.keras.layers.LayerNormalization(epsilon=1e-6,name='norm4')(h_mlp_out + h_hidden)

    h_hidden = GAT(units=6, num_heads=8,name='GAT5')([hin,A])   
    h_hidden = GAT(units=6, num_heads=8, output_layer=True,name='GAT6')([h_hidden,A])   
    h_hidden = tf.keras.layers.LayerNormalization(epsilon=1e-6,name='norm5')(h_hidden + hin)
    h_hidden_reshaped = tf.reshape(h_hidden, [-1, 1581*6])
    #h_hidden = GAT(units=6, num_heads=8)([h_hidden,A])
    h_mlp_in = tf.keras.layers.Dense(2048, activation='relu',name='dense7',kernel_regularizer=l2(0.001))(h_hidden_reshaped)
    h_mlp_hidden = tf.keras.layers.Dense(2048, activation='relu',name='dense8',kernel_regularizer=l2(0.001))(h_mlp_in)
    h_mlp_out = tf.keras.layers.Dense(1581*6, name='dense9')(h_mlp_hidden)
    h_mlp_out = tf.reshape(h_mlp_out, [-1,1581, 6])
...
if __name__ == "__main__":
    ...
    for i in range(int(total_sample_num/sample_for_each_iter)):
        ...
        #model.fit(H_in_norm, H_out, epochs=100, batch_size=20, validation_split=0.2,verbose=1)
        model.fit(H_in_norm, H_out_norm, epochs=30, batch_size=20, validation_split=0.2,verbose=1)

    for i, layer in enumerate(model.layers):
        weights = layer.get_weights()  # 获取层的参数
        np.savez(path+r'/saved_models/1581-sys/GAT_noPI_weights/'+f'layer_{i}_weights.npz', *weights)
...
if __name__ == "__main__":
    model = create_model()
    for i, layer in enumerate(model.layers):
        weights = np.load(path+r'/saved_models/1581-sys/GAT_noPI_weights/'+f'layer_{i}_weights.npz')
        layer.set_weights([weights[f'arr_{j}'] for j in range(len(weights))])

    total_sample_num = 40000
    sample_for_each_iter = 10000
