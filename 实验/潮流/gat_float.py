...
class Attention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        # 将权重转换为复数类型进行计算
        W_complex = tf.cast(self.W, dtype=tf.float32)
        a_1_complex = tf.cast(self.a_1, dtype=tf.float32)
        a_2_complex = tf.cast(self.a_2, dtype=tf.float32)
...
class GraphAttentionLayer(tf.keras.layers.Layer):
    def __init__(self, units, num_heads, output_layer=False, activation=tf.identity, l2=0.0,**kwargs):
...
if __name__ == "__main__":
    ...
    H = np.random.rand(1, num_nodes, num_features)
    A = np.random.rand(1, num_nodes, num_nodes)
