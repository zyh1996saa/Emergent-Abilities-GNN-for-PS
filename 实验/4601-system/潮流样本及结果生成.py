...
def busnum2gennum(bus_num):
    for i in range(ori_case['gen'].shape[0]):
        #print(case_zj['gen'][i,0] , bus_num)
        if ori_case['gen'][i,0] == bus_num + 1:
            return i
...
if __name__ == "__main__":
    ...
    ori_case = case4601.load_case_data()
    ori_case['version'] = '2'
    ori_case = refresh_busnum(ori_case)
    ori_case_res = runpf.runpf(ori_case)
    sys_size = ori_case_res['bus'].shape[0]
...
if __name__ == "__main__":
    ...
    baseMVA = ori_case['baseMVA']
    
    path = r'/home/user/Desktop/预训练大模型'

    for i in range(start,end):
        new_case_i = create_new_case(ori_case,0.2,0.8)
        
        #print(cur_total_netload)
        Hi,Yi,Si = case2AandH(new_case_i)
        sparse_Yi = csr_matrix(Yi)
        np.save(path+r'/数据/潮流(图)格式/%s-system/trainingSet/input/casezj_H_%s.npy'%(sys_size, i), Hi)
        save_npz(path+r'/数据/潮流(图)格式/%s-system/trainingSet/input/casezj_Y_%s'%(sys_size,i), Yi)
        ...
        sparse_Yi = csr_matrix(Yi)
        np.save(path+r'/数据/潮流(图)格式/%s-system/trainingSet/output/casezj_H_%s.npy'%(sys_size,i), Hi)
        save_npz(path+r'/数据/潮流(图)格式%s-system/trainingSet/output/casezj_Y_%s'%(sys_size,i), Yi)
...
if __name__ == "__main__":
    ...
    for i in range(start,end):
        new_case_i = create_new_case(ori_case,0.0,0.2)
        #print(cur_total_netload)
        Hi,Yi,Si = case2AandH(new_case_i)
        sparse_Yi = csr_matrix(Yi)
        np.save(path+r'/数据/潮流(图)格式/%s-system/testSet/input/casezj_H_%s.npy'%(sys_size,i), Hi)
        save_npz(path+r'/数据/潮流(图)格式/%s-system/testSet/input/casezj_Y_%s'%(sys_size,i), Yi)
        ...
        Hi,Yi,Si = case2AandH(res_i)
        sparse_Yi = csr_matrix(Yi)
        np.save(path+r'/数据/潮流(图)格式/%s-system/testSet/output/casezj_H_%s.npy'%(sys_size,i), Hi)
        save_npz(path+r'/数据/潮流(图)格式/%s-system/testSet/output/casezj_Y_%s'%(sys_size,i), Yi)
...
if __name__ == "__main__":
    ...
    for i in range(start,end):
        new_case_i = create_new_case(ori_case,0.8,1.0)
        #print(cur_total_netload)
        Hi,Yi,Si = case2AandH(new_case_i)
        sparse_Yi = csr_matrix(Yi)
        np.save(path+r'/数据/潮流(图)格式/%s-system/testSet/input/casezj_H_%s.npy'%(sys_size,i), Hi)
