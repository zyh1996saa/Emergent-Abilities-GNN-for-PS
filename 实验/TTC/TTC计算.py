...
if __name__ == "__main__":
    ...
    # 受区总负荷
    load_in = [case_zj['bus'][i,2] for i in range(1581) if int(case_zj['bus'][i,0]) in bus_in_list]
    # 受区总限额
    branch_cri = []
    for i in range(case_zj['branch'].shape[0]):
        if (int(case_zj['branch'][i,0]) in bus_in_list) and (int(case_zj['branch'][i,1]) in bus_critical):
            branch_cri.append(i)
        elif (int(case_zj['branch'][i,1]) in bus_in_list) and (int(case_zj['branch'][i,0]) in bus_critical):
            branch_cri.append(i)
    branch_capa_cri = case_zj['branch'][branch_cri,5]
    # 受区总发电
    gen_in = []
    for i in range(case_zj['gen'].shape[0]):
        if int(case_zj['gen'][i,0]) in bus_in_list:
            gen_in.append(i)
    gen_in_sum = case_zj['gen'][gen_in,:1].sum()
    # bus_selected
    ...
    #plt.savefig('subfig_ori.svg',pad_inches = 0,bbox_inches='tight' )
    plt.savefig('subfig_ori_withcolorbar.svg',pad_inches = 0,bbox_inches='tight' )
    plt.show()
    
    maxTTCcasei = find_maxTTC(new_case_i,delta_lambda=0.05,isplot=True,issave=True)
    
    plt = plot_from_case(maxTTCcasei)
    plt.savefig('subfig_end.svg',pad_inches = 0,bbox_inches='tight' )
    plt.show()